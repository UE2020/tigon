...
pub fn encode_positions<B: Position>(pos: &B) -> EncodedPositions {
    ...
    let bishop_difference =
        (bishops & white).count() as isize - (bishops & black).count() as isize + 10;
    let rook_difference = (rooks & white).count() as isize - (rooks & black).count() as isize + 10;
    let queen_difference =
        (queens & white).count() as isize - (queens & black).count() as isize + 9;
