...
fn global_data() -> &'static Mutex<Writer> {
    ...
    INSTANCE.get_or_init(|| {
        Mutex::new(Writer(tensorboard::summary_writer::SummaryWriter::new("logdir/burn-T5"), 0))
    })
}
...
impl<B: Backend> Model<B> {
    ...
    pub fn forward(&self, input: Tensor<B, 4>, mask: Tensor<B, 2>) -> (Tensor<B, 2>, Tensor<B, 2>) {
        ...
        let value = self.value_head.forward(x.detach());
...
impl<B: Backend> ResidualBlock<B> {
    ...
    pub fn forward(&self, input: Tensor<B, 4>) -> Tensor<B, 4> {
        ...
        let x = self.conv1.forward(input.detach());
...
impl<B: ADBackend> TrainStep<PositionBatch<B>, AlphaZeroOutput<B>> for Model<B> {
    fn step(&self, item: PositionBatch<B>) -> TrainOutput<AlphaZeroOutput<B>> {
		...
		let step = writer.1;
		writer.0.add_scalar("Value loss", (item.value_loss.clone()).into_data().value[0].to_f32().unwrap(), step);
		writer.0.add_scalar("Policy loss", (item.policy_loss.clone()).into_data().value[0].to_f32().unwrap(), step);
		writer.0.add_scalar("Training loss", (item.policy_loss.clone()).into_data().value[0].to_f32().unwrap() + (item.value_loss.clone()).into_data().value[0].to_f32().unwrap(), step);
		writer.1 += 1;
